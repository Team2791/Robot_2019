...
public class Elevator extends Subsystem{
    ...
    public Elevator(){
        ...
        poptAnalogInput = new AnalogInput(RobotMap.LIFT_POT_PORT);

        breakSolenoid = new Solenoid(RobotMap.PCM_CAN_ID, RobotMap.BREAK_SOLENOID); // RobotMap ids need to be changed
		breakReleaseTimer = new Timer();

        BaseMotorController leaderTalon = new TalonSRX(RobotMap.LIFT_TALON);
        leaderTalon.configSelectedFeedbackSensor(FeedbackDevice.Analog, 0, 0);
        BaseMotorController followerVictor = new VictorSPX(RobotMap.LIFT_VICTOR);
